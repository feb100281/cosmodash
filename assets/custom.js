...
var dmcfuncs = window.dashMantineFunctions = window.dashMantineFunctions || {};
    ...
    if (absValue >= 1_000_000) {
        ...
        formatted = "₽" + absValue.toFixed(0); // просто число без "тыс" или "млн"
    }
    if (value < 0) {
        formatted = "-" + formatted;
    }
...
dmcfuncs.chartTooltip = ({ label, payload }) => {
  ...
  // автоматически берем все поля кроме 'eom' и тех, что оканчиваются на '_from_first'/'_from_prev'
  const metrics = Object.keys(row)
    .filter(k => !k.endsWith('_from_first') && !k.endsWith('_from_prev') && k !== 'eom')
    .map(k => ({ name: k, key: k }));
//   const metrics = [
//   { name: "Чистая выручка", key: "Чистая выручка" },
//   { name: "Продажи", key: "Продажи" },
//   { name: "Возвраты", key: "Возвраты" },
 
// ];

  return React.createElement(
    dmc.Paper,
    { px: "md", py: "sm", withBorder: true, shadow: "md", radius: "md" },
    [
      React.createElement(
        dmc.Text,
        { key: "tooltip-label", fw: 600, mb: 5 },
        label
      ),
      ...metrics.map((m, idx) =>
        React.createElement(
          ...
          [
            React.createElement(
              dmc.Text,
              { key: `${m.key}-val`, fw: 500 },
              `${m.name}: ${dmcfuncs.formatNumberIntl(row[m.key])}`
            ),
            React.createElement(
              dmc.Text,
              { key: `${m.key}-first`, fz: "xs", c: row[m.key + "_from_first"] >= 0 ? "green.6" : "red.6" },
              `От начала: ${row[m.key + "_from_first"]}%`
            ),
            React.createElement(
              dmc.Text,
              { key: `${m.key}-prev`, fz: "xs", c: row[m.key + "_from_prev"] >= 0 ? "green.6" : "red.6" },
              `От пред.: ${row[m.key + "_from_prev"]}%`
            ),
          ]
        )
      ),
